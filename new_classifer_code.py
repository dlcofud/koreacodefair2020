...
def cnn_classifer():
    ...
    #1
    classifer.add(Conv2D(filters=20, kernel_size=(3, 3), activation = 'relu',input_shape = (64, 64, 1),
                         padding='same',strides=(2,2),kernel_initializer='he_normal'))
    classifer.add(BatchNormalization())
    classifer.add(AveragePooling2D(pool_size=(2,2)))      #globalmaxpooling으로 고침(원래는 maxpooling)
    #classifer.add(BatchNormalization())

    classifer.add(Conv2D(filters=64,kernel_size=(3,3),activation='relu',padding='same',kernel_initializer='he_normal'))
    #classifer.add(BatchNormalization())
    ...
    #네번째, Dense층 추가하기
    classifer.add(Dense(units=64,activation='relu',kernel_initializer='he_normal'))
    classifer.add(Dropout(0.5))
    classifer.add(Dense(units=5,activation='softmax',kernel_initializer='he_normal'))        #시그모이드 함수는 바꾸지 말것
    #이진분류 필요
    classifer.summary()
...
train_set=train_datagen.flow_from_directory('data_pink',target_size=(64,64),color_mode='grayscale',shuffle=True,
                                            classes=['pink_no','pink_1','pink_2','pink_3','pink_4'],
                                            batch_size=batch_size,class_mode='categorical')
#class_mode='binary'는 1차원의 이진 정수라벨이 반환(절대 바꾸지 말것!!!)
#디렉토리의 위치를 전달받아 정규화된 데이터의 배치를 생성
test_set=test_datagen.flow_from_directory('data_pink_test',target_size=(64,64),color_mode='grayscale',shuffle=True,
                                        classes=['pink_no','pink_1','pink_2','pink_3','pink_4'],
                                          batch_size=batch_size,class_mode='categorical')
...
hist=model.fit_generator(train_set,steps_per_epoch=batch_size,epochs=50,validation_data=test_set,
                        shuffle=True,validation_steps=batch_size,callbacks=[reduceLR])          #earlystopping을 추가
